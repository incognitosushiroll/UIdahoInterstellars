...
while True:
    try:
        # Read from serial and parse
        data = ser.read(1)
        #print(data) note that commenting this out enabled the program to work...idk
        slip_driver.receive(data)
        decoded_msg = slip_driver.get(block=False)
        
        if decoded_msg:
            cmd = stt_pb2.StarTrackerCommand()
            cmd.ParseFromString(decoded_msg)
            print(cmd)

            # Process command and compute results
            ...
            out = stt_pb2.StarTrackerOutput(
                ...
                dec=float(dec),
                roll=float(roll),
                match_std_x=float(match_std_x),
                match_std_y=float(match_std_y)
            )

            encoded_output = Driver().send(out.SerializeToString())
            ser.write(encoded_output)
