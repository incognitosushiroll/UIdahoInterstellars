...
cmd.stt_type = "direct_rpi"
cmd.cat_division = 5
cmd.exp_time = 8
#cmd.n_pic = 37

# Wrap in SLIP before sending
slip_driver = Driver() #instantiate driver class from the slip file of the slip library
encoded_cmd = Driver().send(cmd.SerializeToString())
...
while True: 
    try: 
        ...
        decoded_data = slip_driver.get(block=False)  #blocking command to get the buffer until end is reached and outputs a decoded sliplib
